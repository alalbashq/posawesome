  ...
  <div class="customer-input-wrapper">
    <v-autocomplete ref="customerDropdown" class="customer-autocomplete sleek-field" density="compact" clearable
      ...
      <template #append-inner>
        ...
        <v-tooltip text="Add new customer">
          <template #activator="{ on, attrs }">
            <v-icon v-bind="attrs" v-on="on" class="icon-button" @mousedown.prevent.stop @click.stop="load_customers">
              mdi-load
            </v-icon>
          </template>
        </v-tooltip>
...
export default {
  ...
  mounted() {
    var localPosProfile = localStorage.getItem('pos_data');
    if (localPosProfile && localPosProfile !== 'undefined' && localPosProfile !== 'null' && localPosProfile !== '{}' && localPosProfile !== '[]' && this.pos_profile === '') {
      localPosProfile = JSON.parse(localPosProfile);
      this.pos_profile = localPosProfile;
      console.log('register_pos_profile created', localPosProfile);
      this.get_customer_names();
    }
  },
  created() {
    this.$nextTick(() => {
      this.eventBus.on('register_pos_profile', (pos_profile) => {
        ...
        console.log('register_pos_profile created', pos_profile);
        this.get_customer_names();
      });
