<template>
  <div>
    ...
    <v-card style="max-height: 63vh; height: 63vh" class="cards my-0 py-0 mt-3 bg-grey-lighten-5">
      <v-row align="center" class="items px-2 py-1">
        <v-col v-if="pos_profile.posa_allow_sales_order" cols="9" class="pb-0 pr-0">
          <Customer></Customer>
        </v-col>
        <v-col v-if="!pos_profile.posa_allow_sales_order" cols="12" class="pb-0">
          <Customer></Customer>
        </v-col>
      ...
      <v-row align="center" class="items px-2 py-1 mt-0 pt-0" v-if="pos_profile.posa_allow_change_posting_date">
        <v-col v-if="pos_profile.posa_allow_change_posting_date" cols="4" class="pb-0">
      ...
      <div class="my-0 py-0 overflow-y-auto" style="max-height: 45vh">
        <v-data-table
          ...
          >
            ...
            <!-- <v-text-field
              ...
              hide-details
              variant="outlined"
              class="ma-0 pa-0 rate-field"
              @change="calc_prices(item, item.rate)"
            ...
            <template v-slot:item.discount_percentage="{ item }">
              <v-text-field
                ...
                variant="outlined"
                suffix="%"
                class="ma-0 pa-0 discount-field"              
                :disabled="!!item.posa_is_offer || !!item.posa_is_replace "
              />
            </template>

            <!-- Action Buttons -->
            <template v-slot:item.actions="{ item }" :disabled="pos_profile.posa_enable_button_qty">
            <div class="d-flex align-center justify-center" style="gap: 6px;">
              ...
              <v-btn
                ...
                variant="tonal"
                color="primary"
                @click="item.qty > 1 && (item.qty--, calc_stock_qty(item, item.qty))"
                :disabled="!!item.posa_is_offer || !!item.posa_is_replace "
              >
              ...
              <v-btn
                ...
                variant="tonal"
                color="primary"
                @click="item.qty++; calc_stock_qty(item, item.qty)"
                :disabled="!!item.posa_is_offer || !!item.posa_is_replace"
              >
      ...
      <v-row no-gutters>
        <v-col cols="7">
          <v-row no-gutters class="pa-1 pt-2 pr-1">
            ...
            <v-col v-if="pos_profile.posa_use_percentage_discount" cols="6" class="pa-1">
              <v-text-field  v-model="additional_discount_percentage"  @change="update_discount_umount()" :rules="[isNumber]" :label="frappe._('Additional Discount %')" suffix="%" ref="percentage_discount"
...
export default {
  ...
  methods: {
    ...
    update_discount_umount() {
      console.log("Updating additional discount amount", this.additional_discount_percentage);
    ...
    calc_prices(item, $event) {
      console.log("Calculating prices  item:", item);
      console.log("Calculating prices for event:", !$event?.target?.id);
      console.log("Calculating prices for vlue:", !$event?.target?.value);
      if (!$event?.target?.id || !item) return;
      // console.log("Calculating prices for item:", item, value, $event);
  ...
  mounted() {
    ...
    this.eventBus.on("register_pos_profile", (data) => {
      ...
      this.invoiceType = this.pos_profile.posa_default_sales_order
        ? "Order"
        : "Invoice";
      if (this.pos_profile.posa_enable_button_qty) {        
        this.headers.push({
            title: frappe._('Actions'),
            key: 'actions',
            sortable: false,
            align: 'center',
            width: 80
